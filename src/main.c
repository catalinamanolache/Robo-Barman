...
int main(void) {
    ...
    uint16_t last_raw_pot1 = 0;
    uint16_t last_raw_pot2 = 0;
    uint16_t last_raw_pot3 = 0;
    ...
    while (1) {
        ...
        uint8_t vol1_ml = (uint32_t)last_raw_pot1 * MAX_VOL_ML / MAX_ADC_RAW;
        uint8_t vol2_ml = (uint32_t)last_raw_pot2 * MAX_VOL_ML / MAX_ADC_RAW;
        uint8_t vol3_ml = (uint32_t)last_raw_pot3 * MAX_VOL_ML / MAX_ADC_RAW;

        // Apply safety ceilings to ensure values do not exceed 125 ml
        if(vol1_ml > MAX_VOL_ML) vol1_ml = MAX_VOL_ML; 
        if(vol2_ml > MAX_VOL_ML) vol2_ml = MAX_VOL_ML; 
        if(vol3_ml > MAX_VOL_ML) vol3_ml = MAX_VOL_ML;
        ...
        if (system_state == IDLE_STATE) {
            ...
            if ((system_ticks - last_lcd_update) > LCD_REFRESH_RATE) {
                ...
                sprintf(lcd_buffer, "%3d/%3d/%3d ml ", vol1_ml, vol2_ml, vol3_ml);
                LCD_SetCursor(0, 0); 
                LCD_String(lcd_buffer);

                // Validation logic for user input
                if (vol1_ml == 0 && vol2_ml == 0 && vol3_ml == 0) {
                    LCD_SetCursor(1, 0); 
        ...
        // --- STATE 1: SEQUENTIAL POURING ---
            ...
            uint16_t time1_ms = vol1_ml * TIME_MS_PER_ML;
            uint16_t time2_ms = vol2_ml * TIME_MS_PER_ML;
            uint16_t time3_ms = vol3_ml * TIME_MS_PER_ML;
